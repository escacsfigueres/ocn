...
>
> **Parked-items source sweep (2026-05-31):** a source-gated dynamic-workflow sweep graded the parked items (Winawer, Maróczy Bind, Carlsbad, deeper-review eponyms Taimanov/Alapin/Tarrasch/Chigorin). See [`parked-naming-audit-source-sweep.md`](parked-naming-audit-source-sweep.md). Result: **one CLEAR 3-row batch (Maróczy Bind) + 6 PARTIAL.** The Maróczy Bind heads (`B.Sic.Acc.Mar`, `B.Sic.Kan.Mar`, `B.Sic.OKe.c4`) reached a first-hand reference (Edward Winter, *Géza Maróczy*) → apply-ready batch (popularizer; named via Swiderski–Maróczy 1904, he never played it as White). The other six (Winawer, Carlsbad, Taimanov, Alapin, Tarrasch, Chigorin) stay PARTIAL pending one first-hand reference each (mostly the Oxford Companion). **UPDATE 2026-05-31 (reference-source access log, [`parked-attribution-reference-source-log.md`](parked-attribution-reference-source-log.md)):** NotebookLM book passages reached reference-grade for **4 items → CLEAR/apply-ready: `B.Fre.Win` Winawer** (*The Center Game*), **`D.QGD.Exc.Car` Carlsbad** (Karpov & Matsukevich, `historical_notes` only), **`D.Tar` Tarrasch** (Avrukh + *The Power of Pawns*), and **`D.Chi` Chigorin** (Avrukh). The Oxford Companion stayed unreachable. **`B.Sic.Alp` Alapin** and **`B.Sic.Tay` Taimanov** stay PARTIAL — direct nlm re-query returned an explicit negative for each (the agent's Alapin hit was the *Spanish* Alapin, a different opening). **UPDATE: the Maróczy Bind 3-row batch was APPLIED 2026-05-31** (`B.Sic.Acc.Mar`, `B.Sic.Kan.Mar`, `B.Sic.OKe.c4` — attributed_to "Géza Maróczy (popularizer)", strings-only, head rows only; he never played the bind as White, noted in historical_notes).
...
> **Lot 3 ECO-A eponym heads — manifest + dry-run (2026-06-08):** the factory map's recommended first batch (`A.Bir` Bird, `A.Lar` Larsen, `A.Ret` Réti, `A.Gro` Grob) ran through the full pipeline (slice → 4-agent evidence sprint → orchestrator re-verified each Wikipedia source → scaffold → fill → engine `--dry-run --strict --validate`). **All 4 graded CLEAR** (single unambiguous eponym each, no multi-head surname risk; all popularizer/analyst not inventor). Manifest [`manifests/lot-3-eco-a-eponyms.manifest.json`](manifests/lot-3-eco-a-eponyms.manifest.json) passes dry-run (4 rows, validates 5899/0); record [`lot-3-eco-a-eponyms-dry-run.md`](lot-3-eco-a-eponyms-dry-run.md). **APPLIED 2026-06-08** (sha after `3e62cb06…`; 4 rows changed = exactly A.Bir/A.Lar/A.Ret/A.Gro, attribution fields only, zero collateral; validator 5899/0, 140 tests OK, audit unresolved_groups=0). No push yet (separate GO).
