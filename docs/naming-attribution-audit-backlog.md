...
> **Lot A manifest + dry-run (2026-06-08):** the two strongest CLEARs, `D.Tar` (Tarrasch) and `D.Chi` (Chigorin) QGD heads, are now drafted into a real [Attribution Batch Engine](attribution-batch-engine.md) manifest ([`manifests/lot-a-player-eponyms.manifest.json`](manifests/lot-a-player-eponyms.manifest.json)) and **pass `--dry-run --strict --validate`** (2 rows, result validates 5899/0, catalogue untouched) — record in [`lot-a-player-eponyms-dry-run.md`](lot-a-player-eponyms-dry-run.md). **`B.Fre.Win` (Winawer) is held out** of the manifest pending an author/year pin for *The Center Game* (do not conflate with Mario Ziegler). `D.QGD.Exc.Car` (Carlsbad) is a separate Lot B (`historical_notes` only). **No apply** — that is a separate GO.
