#!/usr/bin/env python3
"""Polyglot Zobrist book hash for OCN-1 positions (spec Annex A).

Annex A of the spec makes the **Polyglot book hash** OCN-1's normative
64-bit position key: the XOR of fixed random keys for piece placement,
castling rights, the en-passant file (only when a capture is actually
legal) and the side to move, taken on the position reached by replaying
a row's `moves_uci`. Until roadmap H2.8 the only implementation OCN
could point at lived in a private repo, so the key was documented but
not computable from this checkout. This module computes it here, in
stdlib Python, with no runtime dependency.

Provenance of the key table
---------------------------
`RANDOM_KEYS` below is the 781-entry random array of the **public
Polyglot opening-book format**. It is not a choice OCN made and not a
number OCN may change: the array is part of the format, fixed since
Fabien Letouzey's original `polyglot` distribution, and reproduced
byte-identically across the open implementations (python-chess's
`chess.polyglot.POLYGLOT_RANDOM_ARRAY`, the chessprogramming wiki's
"BookFormats" entry, Stockfish's `book.cpp` lineage, and every `.bin`
book in circulation). The literal here was dumped programmatically from
python-chess 1.11.2 in a throwaway virtualenv that was deleted
afterwards, precisely so that the values are transcription-free; the
module itself is pure stdlib and python-chess is never a dependency of
this repository. `tools/tests/test_polyglot_zobrist.py` pins the result
against the public test vectors published with the book format, so a
corrupted key would fail the suite rather than ship.

Layout of the array, as the format defines it:

    [  0 : 768 ]  piece-square keys, `kind * 64 + rank * 8 + file`
                  with `kind` in the format's own order (black pawn 0,
                  white pawn 1, black knight 2, ... white king 11),
                  rank 0 = rank 1 and file 0 = the a-file
    [768 : 772 ]  castling rights, in the order K Q k q
    [772 : 780 ]  en-passant file, a through h
    [780]         side to move, XORed in when **White** is to move

The en-passant rule
-------------------
Annex A takes the en-passant key only when at least one enemy pawn can
*legally* capture en passant, which is the same rule `fen_key` applies,
so one position has exactly one identity in both encodings. The
classical Polyglot implementations test the weaker condition (an enemy
pawn merely stands beside the pushed pawn) and so disagree in the rare
case where that capture is pinned. Annex A is the normative rule for
OCN; `Board.legal_ep_square()` is where it is enforced, and no
catalogue row is affected by the difference (the divergence needs a
pinned en-passant capture, which no opening tabiya in OCN-1 reaches).

Usage:

    from chess_uci import Board
    from polyglot_zobrist import polyglot_hash, polyglot_hash_after_uci

    polyglot_hash(Board())                    # 5060803636482931868
    polyglot_hash_after_uci("e2e4 c7c5")      # the Sicilian tabiya key
"""
...
try:
    ...
    from tools.chess_uci import Board


#: Offsets into `RANDOM_KEYS`, per the Polyglot book format.
PIECE_BASE = 0
CASTLING_BASE = 768
EN_PASSANT_BASE = 772
TURN_INDEX = 780

#: The format's `kind_of_piece` numbering, keyed by the FEN letter that
#: `chess_uci.Board` stores (upper case = White).
PIECE_KIND = {
    "p": 0, "P": 1,
    "n": 2, "N": 3,
    "b": 4, "B": 5,
...
#: Castling keys are ordered K Q k q, matching the FEN field order.
CASTLING_ORDER = ("K", "Q", "k", "q")

#: The 781 fixed keys of the public Polyglot book format. See the module
#: docstring for provenance; these values are format, not policy.
RANDOM_KEYS = (
...
def polyglot_hash(board: Board) -> int:
    """Return the Polyglot book hash of `board` as an unsigned 64-bit int.

    Implements the XOR scheme of spec Annex A term by term: one
    piece-square key per occupied square, one key per surviving castling
    right, the en-passant file key when (and only when) an en-passant
    capture is legal, and the side-to-move key when White is to move.
    """
    key = 0
    for square, piece in enumerate(board.squares):
        if piece:
            key ^= RANDOM_KEYS[PIECE_BASE + PIECE_KIND[piece] * 64 + square]
    for offset, right in enumerate(CASTLING_ORDER):
        if right in board.castling:
            key ^= RANDOM_KEYS[CASTLING_BASE + offset]
    ep_square = board.legal_ep_square()
    if ep_square is not None:
        key ^= RANDOM_KEYS[EN_PASSANT_BASE + ep_square % 8]
    if board.white_to_move:
        key ^= RANDOM_KEYS[TURN_INDEX]
    return key


def polyglot_hash_after_uci(moves_uci: str) -> int:
    """Replay a UCI line from the initial position and hash the result.

    The companion of `chess_uci.fen_key_after_uci`, for callers that
    hold a move string rather than a board.
    """
    board = Board()
    for token in (moves_uci or "").split():
        try:
            board.push_uci(token)
        except (ValueError, IndexError) as exc:
            raise ValueError(f"illegal UCI move '{token}': {exc}") from exc
    return polyglot_hash(board)
