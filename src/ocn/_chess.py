...
VENDORED from ``tools/chess_uci.py`` — keep the two files in sync. The
copy exists because the installable package must not reach outside its
own tree, while the repository tooling must keep running from a plain
checkout with no install step. A unification (tools importing the
package) is planned; until then, any change here MUST be mirrored in
``tools/chess_uci.py`` and vice versa. Logic is byte-identical below
this docstring.
